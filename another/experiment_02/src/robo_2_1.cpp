...
void Just_move::calculateCommand() {
	...
	if( (&m_laserscan)->ranges.size() > 0)
	  {
		...
		//for real scanner data
		//vector<double> scannerData;
		//for derivative data
	...
	//open a file
	//ofstream myfile;
	//myfile.open("legs.txt", std::ios::out|std::ios::binary);
	...
	for (int i = 0; i < 539; i++) {
		...
		if (deriv_data[i]<-1 && deriv_data[i + 1]>-0.5) {
			...
			//cout << i << "  deriv_data[i]=" << " " << deriv_data[i] << " ";
			//value smaller than -1
			front_leg = i;
			edge_1 = m_laserscan.ranges[i];
			while (deriv_data[i + 1]<1 && i + 1<539) {
				i++;
			}
			//value bigger than 1
			edge_2 = m_laserscan.ranges[i+1];
			//cout << " " << deriv_data[i + 1] << endl;
			//caqlculate the length of leg
			length_leg = sqrt(edge_1*edge_1+ edge_2*edge_2-2*edge_1*edge_2);
			//cout << length_leg << " " << i+1 << " " << "scannerData[i+1]=" << " " << scannerData[i+1] << endl;
			//check which data is leg
			if (length_leg>3 && length_leg<5) {
					scannerData_leg.push_back(front_leg);
					 	scannerData_leg.push_back(i+1);	
				//cout << length_leg <<" " << front_leg << " " << i << endl;
			}
		}
	}
	if(scannerData_leg.size()>0){
		...
		for (int i = 1; i < scannerData_leg.size() - 1 && scannerData_leg.size()>0; i+=2) {
			x_i = scannerData_leg[i];
			y_i = scannerData_leg[i + 1];
			x = m_laserscan.ranges[x_i];
			y = m_laserscan.ranges[y_i];
			distance_between_leg = sqrt(x*x + y*y-2*x*y);
			ROS_INFO( "distance_between_leg =%lf", distance_between_leg);
			//cout << distance_between_leg << " and " << x_i << " and " << y_i << endl;
				if (distance_between_leg < 7 && distance_between_leg >3 && (y_i- x_i<20)) {
				ROS_INFO( "find a person between =%d,=%d", x_i, y_i);
			}
		}


	//change the direction follow person
	double position_middle=(double)(x_i+y_i)*0.5;
		//ROS_INFO(" position_middle=%lf", position_middle);
		//if the wall on the -y direction
		if( position_middle <=265){
		// vel_angular = -(position_middle )*pi / 360; 
	    	vel_angular = -0.2; 
	    	 vel_linear=0.2;
			ROS_INFO(" moving to right <45-265>: distance=%lf,vel_angular=%lf", position_middle,vel_angular);
		}


		if(265 <position_middle && position_middle < 275){
		     vel_angular = 0.0; 
		     vel_linear=0.2;
		ROS_INFO(" moving to right <265-275>: distance=%lf,vel_angular=%lf", position_middle,vel_angular);	
		}

		//if the wall on the +y direction
		if(275<=position_middle ){
		   // vel_angular = (position_middle-270 )*pi / 360; 
		     vel_angular= 0.2;
	   	     vel_linear= 0.2;
		ROS_INFO(" moving to left <275-495>: distance=%lf,vel_angular=%lf", position_middle,vel_angular);
		}
	}
